<div class="card card-body" x-data="{ programs: @entangle('programs'), cabangIdn: @entangle('cabangIdn') }">
    <form wire:submit.prevent="submitForm">
        ...
        <div class="form-group">
            ...
            <input type="radio" id="laki_laki" name="jenis_kelamin" value="Laki-laki" wire:model.defer="jenis_kelamin" required>
            <label for="laki_laki">Laki-laki</label><br>
            <input type="radio" id="perempuan" name="jenis_kelamin" value="Perempuan" wire:model.defer="jenis_kelamin">
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="cabangIdn" wire:model="cabangIdn" required>
                <option value="">Pilih Cabang IDN</option>
                @foreach($cabangIdns as $cabang)
                    <option value="{{ $cabang->id }}">{{ $cabang->nama_cabang }}</option>
                @endforeach
            </select>
            @error('cabangIdn') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        ...
        <div class="form-group" x-show="cabangIdn && programs.length > 0">
            ...
            @error('program_idn') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        ...
        <div class="form-group">
            ...
            @error('bukti_transfer') <span class="text-danger">{{ $message }}</span> @enderror
